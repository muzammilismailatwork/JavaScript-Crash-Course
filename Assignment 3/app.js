(async () => {
  ...
  const setResultItems = (i, data) => {
    ...
    movieDetailPara.innerHTML = `<span style="border:1px solid black; border-radius:3px; padding:2px;">${
      data.certification
    }</span> ${data.genres.map((genre) => genre)} ${Math.floor(
      data.runtime / 60
    )}h${data.runtime % 60}m `;
  ...
  const filterChange = () => {
    let filterData = data.filter((dataObj) => {
      return genreField.value !== "All"
        ? dataObj.genres.includes(genreField.value)
        : dataObj;
    });
    filterData = filterData.filter((dataObj) => {
      return yearField.value !== "All"
        ? dataObj.release_date.includes(yearField.value)
        : dataObj;
    });
    filterData = filterData.filter((dataObj) => {
      return languageField.value !== "All"
        ? dataObj.original_language.includes(languageField.value)
        : dataObj;
    });
    filterData = filterData.filter((dataObj) => {
      return ratingField.value !== "All"
        ? dataObj.vote_average == ratingField.value
        : dataObj;
    });
